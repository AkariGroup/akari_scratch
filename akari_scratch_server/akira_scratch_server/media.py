...
class MediaController:
    ...
    def get_face_result(self) -> GetDetectionResponse:
        res =  GetDetectionResponse()
        if(self._mode == CaptureMode.FACE_DETECTION):
            res.result = True
            res.data = self.capture.get_face_result()
        else:
            res.result = False
            res.data = []
        return res

    def get_object_result(self) -> GetDetectionResponse:
        res =  GetDetectionResponse()
        if(self._mode == CaptureMode.OBJECT_DETECTION):
            res.result = True
            res.data = self.capture.get_object_result()
        else:
            res.result = False
            res.data = []
        return res
    ...
    def _produce(self) -> None:
        while not self._closed:
            ...
            with contextlib.ExitStack() as stack:
                ...
                try:
                    self.capture = capture_factory(mode)
                    stack.enter_context(contextlib.closing(self.capture))
                    self._produce_core(self.capture, mode)
                    self._update_frame(_NOW_LOADING_FRAME, _STATIC_FRAME_STAMP)
                except Exception:
